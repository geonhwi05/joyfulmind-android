
package com.yh04.joyfulmindapp.config;

public class Config {

    public static final String DOMAIN = "https://o0ci4innfc.execute-api.ap-northeast-2.amazonaws.com/dev/";

    public static final String CHATDOMAIN = "https://8ruxmsx29k.execute-api.us-east-1.amazonaws.com/";
    public static final String SP_NAME = "JoyfulMindApp";

    public static final String PLACEDOMAIN = "https://maps.googleapis.com";

    public static final String PLACE_API_KEY = "AIzaSyBzOfk9KZ-o9sKONf_GcUuJ_klNpBkOwJ8";

    public static final String FILE_PROVIDER = "com.yh04.joyfulmindapp.fileprovider";

}